...
import com.plainstride.outbound.core.model.activity.SessionPhase

class WearMainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) { super.onCreate(savedInstanceState); val gateway = WearSessionGatewayImpl(this); setContent { var granted by androidx.compose.runtime.remember { androidx.compose.runtime.mutableStateOf(false) }; val permission = rememberLauncherForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { result -> granted = result[android.Manifest.permission.BODY_SENSORS] == true && result[android.Manifest.permission.ACTIVITY_RECOGNITION] == true && (result[android.Manifest.permission.ACCESS_FINE_LOCATION] == true || result[android.Manifest.permission.ACCESS_COARSE_LOCATION] == true) }; androidx.compose.runtime.LaunchedEffect(Unit) { permission.launch(arrayOf(android.Manifest.permission.BODY_SENSORS, android.Manifest.permission.ACTIVITY_RECOGNITION, android.Manifest.permission.ACCESS_FINE_LOCATION, android.Manifest.permission.ACCESS_COARSE_LOCATION)) }; val state by WearSessionStateStore.state.collectAsStateWithLifecycle(); MaterialTheme { Screen(state, gateway, granted) } } }
}

@androidx.compose.runtime.Composable private fun Screen(state: com.plainstride.outbound.core.model.activity.SessionStateEnvelope?, gateway: WearSessionGateway, granted: Boolean) {
