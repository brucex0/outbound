...
class SpotifyOAuthClient @Inject constructor(@ApplicationContext private val context: Context) : SpotifyAuthorizationClient {
    override suspend fun authorize(): Result<SpotifyAuthorization> {
        ...
        val completion = PendingIntent.getActivity(context, 419, Intent(context, com.plainstride.outbound.MainActivity::class.java).setAction(CALLBACK), PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
    ...
    companion object { const val CALLBACK = "com.plainstride.outbound.spotify.CALLBACK"; private val pending = AtomicReference<CompletableDeferred<Intent>?>(null); fun complete(intent: Intent): Boolean = pending.getAndSet(null)?.let { it.complete(intent) } ?: false }
}
