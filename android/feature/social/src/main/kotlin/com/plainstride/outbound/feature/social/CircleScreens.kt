...
@Composable fun CircleDetailScreen(circle:CircleSummary,close:()->Unit,cheer:(String,String)->Unit,focus:(String,Int?,Boolean)->Unit,archive:()->Unit,invite:()->Unit,rename:(String)->Unit,commitment:(Int?,Boolean)->Unit,primary:()->Unit,mute:(Boolean)->Unit,leave:()->Unit,remove:(String)->Unit)=Dialog(onDismissRequest=close){
 ...
 if(settings)CircleSettingsDialog(circle,{settings=false},rename,primary,mute,archive,leave,remove)
}
