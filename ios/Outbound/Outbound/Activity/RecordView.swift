...
struct RecordView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showsCuratedWorkouts) {
            StandaloneWorkoutPickerView(sport: selectedManualSport ?? .run) { workout in
                ...
                curatedWorkoutIntent = workout.intent
                ...
                selectedWorkoutChoice = .sport(workout.sport)
                selectedGoalMode = .curated
                showsCuratedWorkouts = false
                track(.init(.activityConfigurationChanged, properties: [
                    .changeType: .string("curated_workout"),
    ...
    private var selectedManualSport: SportType? {
        guard case .sport(let sport) = selectedWorkoutChoice else { return nil }
        return sport
    }
    ...
    private var launchGoalCard: some View {
        Button {
            track(.init(.goalEditorOpened, properties: [
    ...
    private func launchWorkoutButton(_ choice: LaunchWorkoutChoice) -> some View {
        ...
        .accessibilityValue(choice.accessibilityValue(
            ...
            curatedIntent: curatedWorkoutIntent,
            selectedMode: selectedGoalMode,
    ...
    private var launchGoalHint: String {
        switch selectedGoalMode {
        case .planned:
            return String(localized: "From your training plan")
        case .curated:
    ...
    private func openCuratedWorkoutPicker() {
        ...
        showsCuratedWorkouts = true
    }

    private func resetCuratedStructureForManualGoalIfNeeded() {
        guard selectedGoalMode == .curated, let sport = selectedManualSport else { return }
        let selectedRoute = plannedIntent?.preparedRoute
        let baseIntent = freestyleFallback(for: sport)

        if let selectedRoute {
            intentBeforeSelectedRoute = baseIntent
            plannedIntent = routeIntent(selectedRoute, appliedTo: baseIntent)
        } else {
            plannedIntent = baseIntent
            intentBeforeSelectedRoute = nil
            selectedRouteDistanceMeters = nil
        }
    }
    ...
    private func trackGoalProgressIfNeeded(_ snapshot: ActiveSessionSnapshot) {
        ...
        where ratio * 100 >= Double(threshold) && reachedGoalThresholds.insert(threshold).inserted {
            track(.init(.goalProgressReached, properties: [
                .activityType: .string((activeIntent ?? plannedIntent ?? .freestyleRun).resolvedActivityType.rawValue),
                .goalType: .string(analyticsGoalType),
    ...
    private var activityConfigurationProperties: [ProductPropertyKey: AnalyticsValue] {
        ...
        return [
            ...
            .activityType: .string(intent.resolvedActivityType.rawValue),
            .goalType: .string(analyticsGoalType),
            .targetBucket: .string(analyticsTargetBucket(for: intent.activityGoal)),
    ...
    private var calorieEditorEstimateLabel: String? {
        ...
        guard let weightKilograms = onboardingStore.latestWeightKilograms else { return nil }
        let estimate: PlannedCalorieEstimate?
        switch selectedManualSport ?? plannedIntent?.sport ?? .run {
        case .run:
            ...
            estimate = WorkoutCalorieEstimator.plannedRun(
                targetCalories: target,
                weightKilograms: weightKilograms,
                paceSecondsPerKilometer: pace
            )
        case .walk:
            estimate = WorkoutCalorieEstimator.plannedWalk(
                targetCalories: target,
                weightKilograms: weightKilograms
            )
        case .bike, .hike, .swim:
    ...
    private func applyGoal(
        _ goal: ActivityGoal,
        trackChange: Bool = true,
        selectionType: String? = nil
    ) -> Bool {
        let currentIntent = plannedIntent ?? .freestyleRun
        if case .calories(let targetCalories) = goal {
            let isEligibleManualSport = selectedWorkoutChoice != .planned
                && (currentIntent.sport == .run || currentIntent.sport == .walk)
            guard currentIntent.allowsCalorieGoal || isEligibleManualSport else {
                showSetupToast(String(
                    localized: "record.goal.calories.activity_unavailable",
                    defaultValue: "Calorie targets are available for manual runs and walks, plus easy or recovery planned runs."
                ))
                return false
            }
            ...
            case .run:
                ...
                estimate = WorkoutCalorieEstimator.plannedRun(
                    ...
                    weightKilograms: weightKilograms,
                    paceSecondsPerKilometer: learnedPace.secondsPerKilometer
                )
                if estimate == nil {
                    showSetupToast(String(
        ...
        if trackChange {
            var properties: [ProductPropertyKey: AnalyticsValue] = [
                ...
                .activityType: .string((plannedIntent ?? currentIntent).resolvedActivityType.rawValue),
                .goalType: .string(analyticsGoalType),
            ...
            if let selectionType {
                properties[.selectionType] = .string(selectionType)
            }
            track(.init(.activityConfigurationChanged, properties: properties))
        }
        return true
    }
    ...
    private func customGoalTextBinding(for kind: CustomGoalKind) -> Binding<String> {
        Binding(
            get: {
                switch kind {
                case .distance: return customDistanceText
                case .time: return customTimeText
                case .calories: return customCaloriesText
                }
            },
            set: { value in
                switch kind {
                case .distance: customDistanceText = value
    ...
    private func customGoalUnitLabel(for kind: CustomGoalKind) -> String {
        switch kind {
        case .distance:
            return measurementPreferences.unitSystem.distanceUnit
        case .time:
    ...
    private func customActivityGoal(for kind: CustomGoalKind) -> ActivityGoal? {
        switch kind {
        case .distance:
            guard let value = Double(customDistanceText.replacingOccurrences(of: ",", with: ".")),
                  value.isFinite,
                  value > 0
            else { return nil }
            return .distanceMeters(measurementPreferences.unitSystem.distanceMeters(from: value))
        case .time:
            guard let minutes = Int(customTimeText),
                  minutes > 0,
                  minutes <= Int.max / 60
            else { return nil }
            return .timeSeconds(minutes * 60)
        case .calories:
            guard let calories = Int(customCaloriesText), calories > 0 else { return nil }
...
private enum LaunchWorkoutChoice: Hashable, Identifiable {
    ...
    func accessibilityValue(
        plannedIntent: SessionIntent?,
        curatedIntent: SessionIntent?,
        selectedMode: SessionGoalMode,
        ...
        case .sport(let sport) where selectedMode == .curated && curatedIntent?.sport == sport:
            curatedIntent?.title ?? String(localized: "Curated")
        case .sport:
...
enum SessionGoalMode: String, CaseIterable, Equatable {
    ...
    var systemImage: String {
        ...
        case .curated: return "sparkles.rectangle.stack"
...
private enum CustomGoalKind: Hashable {
    case distance
    case time
    case calories
}
