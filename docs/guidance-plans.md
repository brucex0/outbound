...
- adaptive plan weeks are seven-day blocks anchored to the plan's creation day, so a plan created late in a calendar week still presents the selected weekly session rhythm as one coherent week; every generated plan workout counts toward that rhythm, while `isKeyWorkout` only identifies the most important training stimulus
- `GET /v1/planning/state` returns the complete current plan-version workout window in `workouts` alongside the bounded future-only `upcoming` list, allowing clients to retain completed-session progress and group the generated 14-day window into two plan weeks
- generated workout schedule timestamps represent logical training dates; clients preserve the UTC year/month/day components when presenting or scheduling them so timezone conversion cannot shift a workout to the preceding local weekday
...
- iOS fetches the plan-independent guided workout catalog from authenticated `GET /v1/planning/standalone-workouts` and caches the last good response for offline use. The server-owned `backend/src/data/standaloneWorkouts.json` catalog defines workout copy, targets, timed steps, source references, prerequisites, safety notes, and structured distance- or time-triggered guide instructions; the app no longer bundles standalone workout definitions. Source material is used as a design reference, while all guide copy is original Plainstride editorial content.
- Starting a standalone workout sends only its catalog ID/version plus the compact client workout summary. The backend resolves the authoritative guide instructions into the existing Gemini start plan, and iOS executes the cached triggers through the same live-coach phrase selection, planned-audio cache, streaming TTS, fallback, cooldown, route-priority, and analytics pipeline used by reactive coaching.
