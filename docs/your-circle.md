...
Open this when designing or building private Circles, weekly shared motivation, Circle management, Circle celebrations, or Circle analytics.

## Product Decision

Plainstride should make running feel good now and easier to return to because effort is noticed by a useful AI companion and shared with a few people who matter.

The core loop is:

`Personal nudge -> commitment -> run -> effort recognized -> Circle progress -> supportive response -> return`

`Your Circle` is the first permanent relationship feature built around that loop. It is a private group for the runner and up to five accepted connections. It is optional and must not weaken the solo experience.

Circle is not a leaderboard, public community, or chat product. It is a lightweight place to stay connected, choose a realistic weekly focus, notice effort, Cheer, and plan a run.
...
- Keep private training, health, location, and motivation context private.
...
- It contains 2–6 accepted members, including the owner.
- Pending invitations do not count toward the member limit until accepted, but the server must prevent invitations that could exceed the limit.
- A runner may belong to multiple Circles. Do not impose a one-Circle database constraint.
- Each runner may choose one primary Circle for Today. The first active Circle becomes primary until changed.
...
1. `personal_targets`: every member independently chooses a run-count target or skips the week.
2. `shared_target`: all qualifying runs contribute toward one shared run-count target.
3. `none`: the Circle has no numeric goal; Cheers and run planning remain available.

The UI should recommend personal targets because they let runners with different abilities contribute fairly. Shared target and no target remain easy alternatives.

The data model must allow future goal types, but the MVP UI supports run count only. Do not build distance, duration, calories, streaks, or ranking goals now.
...
A qualifying contribution is one canonical saved activity whose activity type is running.

- Recorded, imported, and manually entered runs count.
...
- A run that crosses a reset boundary belongs to the week in which it started.
- One canonical activity contributes at most once to a given Circle week.
- If a runner belongs to multiple active Circles, the same run may contribute once to each. Contributions are encouragement, not scarce currency.
...
1. Select 1–5 accepted connections.
...
Keep the existing Today hierarchy and its limit of one social opportunity.

- Show one compact card for the runner's primary active Circle.
- Do not show Circle creation on Today.
- An imminent joined activity event takes priority over Circle progress.
- If the primary Circle has no Weekly Focus, the card may show a recent Cheer or a restrained `Plan a run` action instead of numeric progress.
...
- Circle detail owns member progress, Cheers, run planning, history, and settings.
...
4. `Plan a run`.
5. Recent Circle moments limited to Cheers, joined run plans, and weekly completion—not a duplicate activity feed.
6. Management entry.

Member progress may show `2 of 3`, `Contributed 2`, `Skipping this week`, or no numeric state. Never show pace, route, exact activity time, readiness, or why a member skipped.

## Actions

### Cheer

- A Cheer is a private preset reaction to a member's current Circle-week progress.
- It does not require a public activity post and reveals no source activity.
...
- Store the resulting event through the existing activity-event model; do not create a second Circle-specific run object.
...
## Post-Run And Recognition

After a qualifying run is saved and synchronized, add one compact Circle contribution message to the existing post-run reflection:

```text
This counted for Your Circle.
4 of 6 runs complete this week.
```
...
- A future one-time recognition for a runner's first completed Circle week may use the existing recognition system, but it is not required for MVP.
...
- Historical screens use the display snapshot or remaining share-safe data required by the privacy contract; they must not leak a removed member's private profile data.
...
- relevant planned-run activity-event notifications.
...
Circle members may see only:

- stored Circle name;
- share-safe member name and avatar;
- role;
...
Do not expose through Circle responses or analytics:

- route geometry, coordinates, or meetup data outside an authorized activity event;
- exact activity start time, duration, distance, pace, heart rate, cadence, or calories;
- readiness, injury, cycle, health, or private planning information;
- private reflection, companion prompt, response, inference, or reason for changing/skipping a target;
...
- Cache only share-safe Circle summaries and detail, scoped by authenticated account.
...
`eligible exposure -> creation -> invitation -> acceptance -> active Circle -> focus configured -> run contributed -> Cheer or run planned -> following-week contribution`
...
- progress opened;
- Cheer sent/removed;
- Plan-a-run started/completed;
- run contribution reconciled;
...
- Cheer and Plan-a-run use;
- second- and fourth-week Circle continuity;
- D7 and D28 saved-activity retention;
- active running weeks during the four weeks after Circle activation;
- solo start-to-save conversion as a guardrail;
- mute, leave, block, report, and qualitative guilt/privacy feedback as guardrails.
...
Use one shared product experience and test clarity, warmth, pressure, privacy understanding, and celebration tone with runners aged 18–24, 25–44, and 45–65.
...
- A runner can create a Circle from accepted connections without configuring a goal.
...
- Qualifying canonical runs reconcile exactly once into every applicable Circle week.
- Social and Circle detail show correct combined and member-safe progress.
- Cheer and Plan-a-run work without exposing private activity data.
- The post-run flow presents confirmed contribution without blocking activity save.
...
- Analytics use the typed privacy allowlist and cover the full funnel.
...
- Implement authenticated, membership-authorized Circle APIs with strict block,
  accepted-connection, capacity, privacy, and idempotency enforcement. Return
  canonical state after mutations.
- Reconcile recorded, imported, and manual canonical running activities into the
...
- Add the fast creation/invitation flow, flexible Weekly Focus configuration,
  ...
  run-count target, or no numeric target. Keep Circle and Weekly Focus separate.
...
- Reuse the existing connection picker and activity-event creation flow for Plan
  a run, with Circle members preselected but editable.
- Add private preset Cheers and the required in-app notifications. Do not build
...
- Localize every new iOS string naturally in English, Simplified Chinese, and
  Spanish.
- Add typed, provider-neutral, privacy-allowlisted analytics for the full Circle
