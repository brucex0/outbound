...
- **Before creation — make the idea desirable.** The empty Social card leads with `Live well, together.` and shows walking, running, and cycling symbols. One sentence connects family/friends, workouts, shared weekly momentum, and encouragement. The whole card opens creation.
- **During creation — turn inspiration into one easy decision.** Keep one scrollable screen rather than a wizard: an inspirational hero, `Who helps you keep moving?`, the accepted-connection picker, an optional name, and one persistent `Create your Circle` action. Show the number selected but do not advertise a fixed maximum; disable additional choices only when the backend-delivered capacity is reached.
...
- Show one compact card for the member's primary active Circle.
