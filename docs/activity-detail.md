...
The Share Activity Card action exports a 9:16 image built from a full-bleed muted `MKMapSnapshotter` route screenshot with the recorded route drawn over it and key stats overlaid in a Strava-style lower gradient. The lower-right branding includes a scannable QR code for a newly created referral link. The system Share Sheet receives only the PNG card so Copy and messaging apps cannot mistake a secondary URL payload for an extensionless attachment; the referral remains available through the QR code. If referral creation is unavailable, the QR code uses the stable `https://run.plainstride.com/invite` download link; if a route snapshot cannot be created, the export falls back to a stats-only card so sharing still succeeds.
