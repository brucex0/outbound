...
`backend/src/routes/circles.ts` and `backend/src/services/circles.ts` own the permanent invitation-only Circle slice. Prisma stores lifecycle, server-owned invitations, membership snapshots/preferences, immutable Circle-week intervals, optional focus configuration, personal commitments, activity-backed contributions, one-time completion presentation, preset Cheers, and the account-owned primary Circle selection. All Circle reads and mutations derive identity from authentication, enforce active membership or a valid invitation, and recheck accepted connections, blocks, and each Circle's snapshotted member limit. `CIRCLE_MEMBER_LIMIT` configures new-Circle capacity and the list response exposes the current creation policy to clients.

`POST /v1/activities` and activity deletion invoke the idempotent Circle reconciler after the canonical activity operation. The reconciler assigns all canonical activity types by actual start time, permits one contribution per Circle week, handles multiple active Circles, and recomputes deletions. Its failures are logged and never fail activity save or deletion. Authenticated Circle responses include the trusted workout summary defined in `docs/your-circle.md` for the most recent current-week activity per member; internal activity IDs, routes, reflections, and companion context stay server-side.
