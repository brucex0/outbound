...
For the canonical production target, the deploy script defaults to the already-approved live-coach production profile. This prevents an ordinary backend code deploy from silently replacing active production settings with the original pre-rollout safety defaults. Non-production project/service targets still default live coaching and the planner to disabled, `en` and `zh-Hans`, the Google Cloud TTS route enabled, one female and one male product voice, and dynamic rollout at zero. Google TTS and Vertex AI authenticate through the attached Cloud Run runtime service account.
