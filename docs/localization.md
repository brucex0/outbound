...
- Route generated companion announcements through iOS speech synthesis. Settings lists only voices compatible with the active app language: every installed Apple Premium and Enhanced voice for selection and on-device preview, plus one deterministic Standard fallback under its actual system name. Warn before selecting the Standard voice because it may sound poor. When the app language changes, require a new explicit voice choice instead of silently accepting an incompatible or Standard fallback. Speech must use the selected voice's identifier, locale, configured rate, and volume.
