...
- Your Circle UI, formatted progress, preset Cheers, transient results, accessibility labels, and semantic in-app notification copy use `circle.*` String Catalog entries in English, Spanish, and Simplified Chinese. Generated default Circle names are resolved server-side in the creator's canonical locale and then stored as shared user-visible content.
