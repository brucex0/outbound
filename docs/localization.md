...
- Group UI, formatted progress, preset Cheers, transient results, accessibility labels, and semantic in-app notification copy use `group.*` String Catalog/resource entries in English, Spanish, and Simplified Chinese. Generated default Group names are resolved server-side in the creator's canonical locale and then stored as shared user-visible content.
