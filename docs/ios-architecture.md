...
- `Features/Simplified/SimplifiedAppShell.swift`: primary `Social · Today · Me` shell. It hides the legacy assistant/activity chrome and continues to reuse the retained `RecordView` overlay when Today starts a run. The Me profile editor updates the user's name, bio, contact email, contact phone, and photo; contact details are profile data and do not change Firebase sign-in credentials. Apply the contact-field schema addition with `cd backend && npm run db:push` before using this editor against an existing backend database. After an activity is saved on the current local day, Today replaces the planned-workout emphasis with a completion card, summary stats, and a direct activity-detail link while retaining Run Again and Quick Start. Today invalidates cached companion guidance when the displayed workout changes, at local-day rollover, or on foreground resume; companion requests explicitly identify the visible workout so insight cannot refer to an earlier plan day. The companion backend uses the client timezone for current-day and current-week context boundaries. Debug launches can temporarily use `-OutboundLegacyShell` for regression comparison; release builds always use the simplified shell.
...
- `DesignSystem/OutboundDesignSystem.swift`: initial target design tokens and reusable card, primary-button, AI-explanation, and workout-phase summary components. New simplified-product features should extend these components instead of adding styles to legacy monolithic screens.
