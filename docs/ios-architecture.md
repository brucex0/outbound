...
- `Features/Simplified/SimplifiedAppShell.swift`: first replacement-by-slice scaffold for the target `Together · Today · Me` shell. Debug launches with `-OutboundSimplifiedShell` show the new shell, skip the legacy onboarding presentation, hide legacy assistant/activity chrome, and continue to reuse the retained `RecordView` overlay when Today starts a run.
