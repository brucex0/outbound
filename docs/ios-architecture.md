...
- `Features/Simplified/SimplifiedAppShell.swift`: primary `Together · Today · Me` shell. It hides the legacy assistant/activity chrome and continues to reuse the retained `RecordView` overlay when Today starts a run. Debug launches can temporarily use `-OutboundLegacyShell` for regression comparison; release builds always use the simplified shell.
...
- `Features/Simplified/SimplifiedAppShell.swift`: Today renders local conditions as one compact row inside the workout card. The detail sheet explains any weather guidance, forecast privacy, and Apple Weather attribution; unavailable weather never blocks the workout.
