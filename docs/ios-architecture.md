...
- `Domains/Athlete/PersonalizationContracts.swift`: versioned Codable contracts for calibration, runner insights with confidence, readiness, post-run feedback, and explainable plan adjustments. The simplified Today scaffold uses the typed preview snapshot while backend integration is built.
