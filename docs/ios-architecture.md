...
- `Integrations/Weather/SituationalWeatherStore.swift`: opt-in WeatherKit integration for Today. It requests a one-shot reduced-accuracy location, caches a normalized 30-minute snapshot, derives bounded running guidance and a near-term outdoor window, and exposes an expiring situational signal to the companion without sending coordinates. Weather guidance can recommend a small adjustment but does not mutate the plan.
- `Features/Simplified/SimplifiedAppShell.swift`: Today renders local conditions as one compact row inside the workout card. The detail sheet explains any weather guidance, forecast privacy, and Apple Weather attribution; unavailable weather never blocks the workout. The daily companion response is also collapsed into a compact visual indicator and reveals its full explanation only when tapped.
