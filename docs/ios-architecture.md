...
- `Coach/CoachSpeechEngine.swift`: pluggable coach speech layer. `AppleSystemSpeechEngine` is the active shipping path and now prints voice inventory/selection diagnostics to the device console. `PiperSpeechEngine` is the local-TTS integration point and checks for bundled Piper runtime/model assets before falling back.
- `Coach/VirtualCoach.swift`: consumes `ActiveSessionSnapshot` values while active, keeps a rolling history, requests analysis after 20 seconds and then at the selected persona's nudge frequency, seeds the first message from the selected session intent when present, displays `lastNudge`, suppresses recently repeated spoken lines, and routes spoken output through the pluggable coach speech engine while preserving the spoken-audio `AVAudioSession` lifecycle.
...
- `App/OutboundApp.swift`: now also defines the local-first training plan models and `TrainingPlanStore`, which handles predefined plan recommendations, active-plan state, weekly progress, and today's adaptive plan suggestion.
- `App/TrainingPlanLibrary.swift`: local structured plan library. It stores `TrainingPlanTemplate`, `TrainingPlanWeek`, `TrainingPlanWorkout`, and `TrainingPlanWorkoutStep` content, including imported MIT Couch-to-5K data, imported MIT `time-to-run` base and half-marathon plans, and authored 10K, 10 mile, half marathon, consistency, and comeback plans. The file also contains the normalization helpers that convert imported plain-text workout descriptions into Outbound's structured workout model.
- `App/MainTabView.swift`: now also contains the motivation MVP types, `MotivationDashboardView`, and `TrainingPlanCard`, including spark, focus setup/progress, plan recommendations or active plan, check-in, suggested actions, momentum strip, plan detail pages, and the local engine that derives motivation state and finish reflections.
