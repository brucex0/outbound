...
- `Features/Simplified/SimplifiedAppShell.swift`: primary `Social · Today · Me` shell. It hides the legacy assistant/activity chrome and continues to reuse the retained `RecordView` overlay when Today starts a run. After an activity is saved on the current day, Today replaces the planned-workout emphasis with a completion card, summary stats, and a direct activity-detail link while retaining Run Again and Quick Start. Today companion guidance invalidates when the local completed activity changes, refreshes again when opened, and includes the local completion details so it does not wait for backend activity sync. Debug launches can temporarily use `-OutboundLegacyShell` for regression comparison; release builds always use the simplified shell.
