...
- `Coach/VirtualCoach.swift`: consumes `ActiveSessionSnapshot` values while active, keeps a rolling history, requests analysis after 20 seconds and then at the selected persona's nudge frequency, seeds the first message from the selected session intent when present, displays `lastNudge`, speaks nudges with the selected voice settings, and owns the spoken-audio `AVAudioSession` lifecycle so background coach announcements duck other audio and release it cleanly afterward.
