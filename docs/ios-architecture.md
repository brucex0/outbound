...
- `Activity/RecordView.swift`: owns the shared activity start page and recording flow. Structured planned workouts show a read-only phase-by-phase workout plan so their steps cannot be accidentally replaced by the freestyle goal editor; Quick Start and simple distance/time launches retain the editable goal setup. The start page can also arm live sharing and mark the session as treadmill/indoor before Start. When live sharing is armed, Start creates the server share, opens the system Share Sheet with trusted-contact copy, then begins the countdown after the sheet is dismissed. Start otherwise opens the live camera/map surface, runs a short visual and spoken countdown through `VirtualCoach`, then starts `ActivityRecorder` on "Go" so elapsed time and GPS begin after the countdown. During the activity it forwards live snapshots to the coach, collects captured photos, and presents the reflection-first Save/Discard sheet after finish. A top `chevron.down` hides the page without stopping an active session, and the bottom-row activity button reopens it.
