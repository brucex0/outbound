...
- `Activity/RecordView.swift`: owns the Record tab state. Creates `LocationManager` and `ActivityRecorder`, shows the pre-activity Start screen inside the main three-tab shell, opens the live camera/map recorder after Start, activates `VirtualCoach` with `coachCatalog.selectedPersona`, forwards live snapshots to the coach, collects captured photos during the activity, and presents the Save/Discard sheet after finish. Saving an activity also persists the route needed for map display and later export.
