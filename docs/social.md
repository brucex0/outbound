...
The signed-in runner's Me and Profile cards expose an owner-only QR action. Connections' add menu also provides Scan QR code, Show my QR code, and Invite by link. The full-screen camera scanner accepts only canonical `https://run.plainstride.com/connect/:code` URLs, pauses while it submits a recognized code, and routes the result through the existing transient Social toast. Denied camera access provides a Settings recovery action, while unsupported devices receive an explicit unavailable state. The QR uses the account's existing opaque referral token through the distinct `/connect/:code` Universal Link path; scanning it after authentication creates a normal pending connection request to the QR owner. It never auto-accepts a connection. Self scans, existing pending requests, accepted connections, blocks, invalid codes, and retryable network failures resolve idempotently without exposing account IDs in the QR or analytics.
