...
Social home has a persistent Connections section rather than burying Connections in the community menu. Its first page is prefetched with the other startup Social data and cached per account; when no snapshot exists yet, a footprint-matched placeholder keeps the sections below it stationary while the request completes. With accepted connections, it shows a compact profile preview ordered by active workout first and then first name. Active workout presence starts with the recorder, remains active while recording is active or paused, refreshes once per minute, ends explicitly with the workout, and expires server-side after three minutes without a heartbeat. The connection response exposes only a boolean to accepted connections—never workout metrics, location, goal, timestamps, or session IDs. With no accepted connections, the section becomes the Better Together growth card. The full Connections screen supports authenticated listing, name/username search, requests, acceptance, decline/cancel, and removal. Search accepts a single Unicode character, normalizes compatibility forms before lookup, and submits committed keyboard input as well as debounced changes, so Chinese names work without a Latin-centric minimum length. One- and two-character queries remain literal; longer queries use indexed trigram matching so minor typos can still find a display name or username. Exact, prefix, and substring matches rank ahead of fuzzy results. Search remains within the entered script and does not transliterate identities between scripts. Its server list uses a stable opaque cursor ordered by relationship update time and ID, and the screen loads 20 relationships at a time. Pull-to-refresh resets the list to its newest page; event invitation pickers exhaust the remaining pages so eligible friends are not silently omitted. Pending incoming requests also appear as contextual shortcuts on Social home. Referrals remain invitation links and do not silently create a connection.
...
The signed-in runner's Me and Profile cards expose an owner-only QR action. Connections' add menu also provides Scan QR code, Show my QR code, and Invite by link. Android's full-screen camera scanner accepts only canonical `https://run.plainstride.com/connect/:code` URLs and resolves a recognized code through the same read-only profile preview as an Android App Link. Opening that recognized link through an iOS Universal Link or Android App Link resolves the opaque code without mutation, opens the owner's Social profile, and lets the viewer explicitly Connect; it never sends a request merely because the app opened. Denied camera access provides a Settings recovery action, while unsupported devices receive an explicit unavailable state. The QR uses the account's existing opaque referral token through the distinct `/connect/:code` path. The authenticated read-only lookup returns only the owner's existing Social identity and the viewer's relationship state; blocked and invalid links remain indistinguishable as not found. Explicit Connect creates a normal pending request and never auto-accepts. Self links, existing pending requests, accepted connections, blocks, invalid codes, and retryable network failures remain idempotent without exposing account IDs in the QR or analytics.

The Social home connection-growth card is limited to runners with no accepted connections and remains hidden until the first connection snapshot loads successfully, so an unknown empty state never flashes the prompt. Connections keeps people discovery in its always-visible inline search field. Debounced matches appear in a floating autocomplete panel directly below the field instead of being appended to the relationship list. Submitting from the keyboard opens a dedicated Search people screen with full results and relationship actions; autocomplete and submitted-result states remain visually separate from Requests, Connections, Sent, and Blocked. The top-right add menu owns QR scanning, the owner QR, and referral-link sharing. Upcoming includes a visible Discover entry for browsing compatible activities from connections and joined groups. Upcoming and Recent sections remain visible with explicit empty cards when they have no content.
