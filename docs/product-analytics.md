...
- Startup routing emits `app_startup_resolved` once per process launch with only the resolved destination (`authentication`, `onboarding`, or `main`) and a coarse latency bucket. It excludes account identifiers, onboarding answers, session details, and exact timing.
