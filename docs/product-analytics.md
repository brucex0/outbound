...
- Planning navigation emits `planning_surface_opened` with only a bounded surface (`curated_workouts`, `planned_workout_details`, `plan_details`, or `plan_picker`) and entry source (`manual_mode_row`, `today_planned_card`, `today_planned_card_plan`, `today_planned_card_change`, `me_current_focus`, `plan_details_change`, or `recommendation_more_plans`). Choosing a curated workout additionally emits `activity_configuration_changed` with `change_type = curated_workout` and only the selected sport as `selection_type`. These events exclude workout and plan IDs, titles, details, recommendations, and runner data.
