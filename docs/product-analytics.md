...
- Authentication identity is synchronized at the app root with the opaque Plainstride account ID and cleared when the authenticated user disappears. Firebase collection is disabled before authentication and enabled only after that account ID is set, so anonymous app-open/session activity does not inflate registered-user dashboards.
