...
- Run-goal profile changes reuse `preference_changed` with bounded kinds (`primary_motivation` or `run_goal_type`) and enum selections only. One-off goal changes reuse `activity_configuration_changed` with coarse calorie target buckets and bounded source (`manual` or `planned_override`). These events never include weight, exact calories, free-form motivation, workout IDs, learned pace, or estimated distance/duration.
