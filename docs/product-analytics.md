...
- The assistant launcher motion experiment emits `assistant_launcher_eligible_exposure`, `assistant_launcher_animation_shown`, `assistant_launcher_opened`, and `assistant_meaningful_engagement`. Every event contains only the persisted local experiment variant, current top-level destination, and a bounded entry source. Meaningful engagement means selecting a suggestion or sending a message; prompts, assistant responses, workout details, and other personal activity data are excluded.
...
| Assistant launcher experiment | eligible exposure -> optional treatment animation shown -> launcher opened -> suggestion selected or message sent |
