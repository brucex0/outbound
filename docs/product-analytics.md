...
- The activity flow emits setup exposure, configuration, start/pause/resume/finish/save/discard, goal-threshold, music, route-selection, shoe-selection, photo, and group-run events. Interrupted-workout music recovery emits only success/failure plus the bounded recovery source (`already_playing`, `existing_queue`, `rebuilt_selection`, or `unavailable`), never playlist or track metadata.
