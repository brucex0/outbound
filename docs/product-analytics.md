...
- Account recognition reconciliation emits `recognition_sync_completed` with only a bounded source type and coarse award-count bucket, or `recognition_sync_failed` with a bounded source type and normalized error category. It never includes badge IDs, activity IDs, earn dates, profile IDs, or raw server errors.
