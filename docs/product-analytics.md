...
- Live Guidance speech-route experiments also emit only coarse provider source (`remote` or `local`) and result (`success` or `fallback`); provider/model/voice IDs, exact transcripts, and prompts are excluded.
