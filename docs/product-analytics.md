...
- Activity finish also emits one `activity_recording_quality` event with only a coarse accepted-point-count bucket, whether Core Location used single or batched delivery, and whether some fixes were filtered. It never includes coordinates, timestamps, exact point counts, or route geometry.
