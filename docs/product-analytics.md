...
- Subscription surfaces emit `subscription_paywall_opened` or `subscription_customer_center_opened` with only `entry_source = rewards_center`. Purchase, restore, and Customer Center dismissal emit `subscription_reconciled` with only a bounded source and success/failure. Product identifiers, prices, receipts, transaction IDs, RevenueCat customer data, and entitlement dates never enter product analytics.
